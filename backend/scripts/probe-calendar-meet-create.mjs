/**
 * Prueba aislada: DWD + calendar.events con Google Meet.
 *
 *   node backend/scripts/probe-calendar-meet-create.mjs
 */
...
const ATTENDEE = (process.argv[2] ?? 'admin@bacarsa.com.ar').trim().toLowerCase()
...
async function main() {
  ...
  console.log('=== Probe Calendar Meet DWD ===')
  ...
  const requestBody = {
    ...
    description: [
      'Prueba aislada de calendar.events + Google Meet.',
      '',
      `Impersonando: ${SUBJECT}`,
      `Invitado: ${ATTENDEE}`,
      `Creado: ${stamp}`,
      'Script: backend/scripts/probe-calendar-meet-create.mjs',
    ].join('\n'),
    start: {
    ...
    conferenceData: {
      createRequest: {
        requestId: `bacarnet-probe-${Date.now()}`,
